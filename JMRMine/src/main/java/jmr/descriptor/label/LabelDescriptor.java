...
public class LabelDescriptor<T> extends MediaDescriptorAdapter<T> implements Serializable {
     ...
     */
    static private Comparator DEFAULT_COMPARATOR = new EqualComparator();
    /**
     * Comparator used by default.
     */
    static private Comparator DEFAULT_WEIGHTED_COMPARATOR = new WeightBasedComparator();
    /**
     ...
     * included)
     */
    //private Double inclusionDistance(LabelDescriptor u, UnaryOperator<Double> op_init, BinaryOperator<Double> op_aggregation) {
    private Double inclusionDistance(LabelDescriptor u, int type) {
        int equal;
        String label_i;
        ...
        for (int i = 0; i < this.size(); i++) {
            ...
            if (equal != 0) {
                ...
                switch(type) {
                    case WeightBasedComparator.TYPE_MAX:
                        dist = dist == null ? dist_ij : Math.max(dist, dist_ij);
                        break;
                    case WeightBasedComparator.TYPE_MIN:
                        dist = dist == null ? dist_ij : Math.min(dist, dist_ij);
                        break;
                    case WeightBasedComparator.TYPE_MEAN:
                        dist = dist == null ? dist_ij : dist + dist_ij;
                        break;
                    case WeightBasedComparator.TYPE_EUCLIDEAN:
                        dist = dist == null ? dist_ij : dist + dist_ij*dist_ij;
                        break;
                }
        ...
        if (type == WeightBasedComparator.TYPE_MEAN) {
            dist = dist / this.size();
        }
        else if (type == WeightBasedComparator.TYPE_EUCLIDEAN) {
            dist = Math.sqrt(dist);
        }
    ...
    static public class WeightBasedComparator implements Comparator<LabelDescriptor, Double> {
        ...
        public Double apply(LabelDescriptor t, LabelDescriptor u) {
            ...
            Double output = t.inclusionDistance(u, this.type);
            return output;
        }
    }
